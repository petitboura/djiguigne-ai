...
// La catégorie choisie ici est transmise via ?categorie=<id>, lue par
// PageCreerAgent pour la présélectionner.
export function BoutonDevenirCreateur({
  ...
  const [etape, setEtape] = useState<"ferme" | "explication" | "categorie">("ferme");

  useEffect(() => setMonte(true), []);

  function choisirCategorie(categorie: Categorie) {
    setEtape("ferme");
    router.push(`/dashboard/agents/nouveau?categorie=${categorie.id}`);
  }
  ...
  return (
    <>
      ...
      <PopupCategories
        ouvert={etape === "categorie"}
        onFermer={() => setEtape("ferme")}
